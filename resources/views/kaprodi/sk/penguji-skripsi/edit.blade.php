...
@section('title', 'Edit SK Penguji Skripsi')
...
<div class="mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('kaprodi.sk.penguji-skripsi.index') }}">SK Penguji Skripsi</a></li>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-warning text-dark">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="mb-0 fw-bold">Edit & Ajukan Ulang SK Penguji Skripsi</h5>
                        <small>Perbaiki data dan ajukan kembali SK yang ditolak</small>
                    </div>
                </div>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('kaprodi.sk.penguji-skripsi.update', $sk->No) }}" method="POST" id="formPengujiSkripsi">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="table-responsive" style="overflow: visible;">
                            <table class="table table-bordered table-hover" id="tabelPenguji">
                                ...
                                <tbody id="bodyPenguji">
                                    @php
                                        $rawData = $sk->Data_Penguji_Skripsi;
                                        $pengujiData = is_string($rawData) ? json_decode($rawData, true) : (is_array($rawData) ? $rawData : []);
                                    @endphp
                                    @foreach($pengujiData as $index => $data)
                                    @php
                                        $mhs = $mahasiswas->firstWhere('Id_Mahasiswa', $data['mahasiswa_id'] ?? null);
                                        $penguji1 = $dosens->firstWhere('Id_Dosen', $data['penguji_1_id'] ?? null);
                                        $penguji2 = $dosens->firstWhere('Id_Dosen', $data['penguji_2_id'] ?? null);
                                        $penguji3 = $dosens->firstWhere('Id_Dosen', $data['penguji_3_id'] ?? null);
                                    @endphp
                                    <tr data-index="{{ $index + 1 }}">
                                        ...
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       data-row="{{ $index + 1 }}"
                                                       value="{{ $mhs->Nama_Mahasiswa ?? ($data['nama_mahasiswa'] ?? '') }}"
                                                       placeholder="Ketik nama mahasiswa..." 
                                                       autocomplete="off"
                                                       required>
                                                <input type="hidden" name="penguji[{{ $index + 1 }}][mahasiswa_id]" class="mahasiswa-id-hidden" value="{{ $data['mahasiswa_id'] ?? '' }}" required>
                                        ...
                                        <td>
                                            <input type="text" 
                                                   ...
                                                   readonly 
                                                   value="{{ $mhs->NIM ?? ($data['nim'] ?? '') }}"
                                                   placeholder="NIM">
                                        </td>
                                        <td>
                                            <textarea class="form-control form-control-sm" 
                                                      name="penguji[{{ $index + 1 }}][judul_skripsi]" 
                                                      rows="2" 
                                                      placeholder="Judul skripsi"
                                                      required>{{ $data['judul_skripsi'] ?? '' }}</textarea>
                                        </td>
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       data-row="{{ $index + 1 }}"
                                                       data-penguji="1"
                                                       value="{{ $penguji1->Nama_Dosen ?? ($data['nama_penguji_1'] ?? '') }}"
                                                       placeholder="Ketik nama dosen..." 
                                                       autocomplete="off"
                                                       required>
                                                <input type="hidden" name="penguji[{{ $index + 1 }}][penguji_1]" class="dosen-id-hidden" value="{{ $data['penguji_1_id'] ?? '' }}" required>
                                                <div class="autocomplete-suggestions"></div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       data-row="{{ $index + 1 }}"
                                                       data-penguji="2"
                                                       value="{{ $penguji2->Nama_Dosen ?? ($data['nama_penguji_2'] ?? '') }}"
                                                       placeholder="Ketik nama dosen..." 
                                                       autocomplete="off"
                                                       required>
                                                <input type="hidden" name="penguji[{{ $index + 1 }}][penguji_2]" class="dosen-id-hidden" value="{{ $data['penguji_2_id'] ?? '' }}" required>
                                                <div class="autocomplete-suggestions"></div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       data-row="{{ $index + 1 }}"
                                                       data-penguji="3"
                                                       value="{{ $penguji3->Nama_Dosen ?? ($data['nama_penguji_3'] ?? '') }}"
                                                       placeholder="Ketik nama dosen..." 
                                                       autocomplete="off"
                                                       required>
                                                <input type="hidden" name="penguji[{{ $index + 1 }}][penguji_3]" class="dosen-id-hidden" value="{{ $data['penguji_3_id'] ?? '' }}" required>
                                                <div class="autocomplete-suggestions"></div>
                                            </div>
                                        </td>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('kaprodi.sk.penguji-skripsi.index') }}" class="btn btn-secondary">
                            <i class="fas fa-arrow-left me-2"></i>Kembali
                        </a>
                        <button type="submit" class="btn btn-warning" id="btnSubmit">
                            <i class="fas fa-paper-plane me-2"></i>Ajukan Ulang SK
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    let rowIndex = {{ count($pengujiData) }};
    ...
    $(document).ready(function() {
        ...
        $('#bodyPenguji tr').each(function() {
            const mahasiswaInput = $(this).find('.mahasiswa-autocomplete')[0];
            initMahasiswaAutocomplete(mahasiswaInput);
            
            $(this).find('.dosen-autocomplete').each(function() {
