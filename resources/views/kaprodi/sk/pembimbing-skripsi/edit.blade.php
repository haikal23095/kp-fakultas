...
@section('title', 'Edit SK Pembimbing Skripsi')
...
<div class="mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('kaprodi.sk.pembimbing-skripsi.index') }}">SK Pembimbing Skripsi</a></li>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-warning text-dark">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="mb-0 fw-bold">Edit & Ajukan Ulang SK Pembimbing Skripsi</h5>
                        <small>Perbaiki data dan ajukan kembali SK yang ditolak</small>
                    </div>
                </div>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('kaprodi.sk.pembimbing-skripsi.update', $sk->No) }}" method="POST" id="formPembimbingSkripsi">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover" id="tabelPembimbing">
                                ...
                                <tbody id="bodyPembimbing">
                                    @php
                                        $rawData = $sk->Data_Pembimbing_Skripsi;
                                        $pembimbingData = is_string($rawData) ? json_decode($rawData, true) : (is_array($rawData) ? $rawData : []);
                                    @endphp
                                    @foreach($pembimbingData as $index => $data)
                                    @php
                                        $mhs = $mahasiswas->firstWhere('Id_Mahasiswa', $data['id_mahasiswa'] ?? null);
                                        // Handle both nested object and direct ID format
                                        $pembimbing1Id = is_array($data['pembimbing_1'] ?? null) ? ($data['pembimbing_1']['id_dosen'] ?? null) : ($data['pembimbing_1'] ?? null);
                                        $pembimbing2Id = is_array($data['pembimbing_2'] ?? null) ? ($data['pembimbing_2']['id_dosen'] ?? null) : ($data['pembimbing_2'] ?? null);
                                        $pembimbing1 = $dosens->firstWhere('Id_Dosen', $pembimbing1Id);
                                        $pembimbing2 = $dosens->firstWhere('Id_Dosen', $pembimbing2Id);
                                    @endphp
                                    <tr data-index="{{ $index + 1 }}">
                                        ...
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       autocomplete="off">
                                                <input type="hidden" 
                                                       name="pembimbing[{{ $index + 1 }}][mahasiswa_id]" 
                                        ...
                                        <td>
                                            <textarea class="form-control form-control-sm" 
                                                      name="pembimbing[{{ $index + 1 }}][judul_skripsi]" 
                                                      rows="2" 
                                        ...
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       ...
                                                       value="{{ $pembimbing1->Nama_Dosen ?? (is_array($data['pembimbing_1'] ?? null) ? ($data['pembimbing_1']['nama_dosen'] ?? '') : '') }}"
                                                       placeholder="Ketik nama/NIP..."
                                                       autocomplete="off"
                                                       data-target="pembimbing_1">
                                                <input type="hidden" 
                                                       name="pembimbing[{{ $index + 1 }}][pembimbing_1]" 
                                        ...
                                        <td>
                                            <div class="autocomplete-wrapper">
                                                <input type="text" 
                                                       class="form-control form-control-sm dosen-autocomplete" 
                                                       value="{{ $pembimbing2->Nama_Dosen ?? (is_array($data['pembimbing_2'] ?? null) ? ($data['pembimbing_2']['nama_dosen'] ?? '') : '') }}"
                                                       placeholder="Ketik nama/NIP..."
                                                       autocomplete="off"
                                                       data-target="pembimbing_2">
                                                <input type="hidden" 
                                                       name="pembimbing[{{ $index + 1 }}][pembimbing_2]" 
                                                       class="dosen-id-hidden"
                                                       value="{{ $pembimbing2Id ?? '' }}">
                                                <div class="autocomplete-suggestions dosen-suggestions"></div>
                                            </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('kaprodi.sk.pembimbing-skripsi.index') }}" class="btn btn-secondary">
...
<script>
    let rowIndex = {{ count($pembimbingData) }};
    ...
    $(document).ready(function() {
        ...
        $('#bodyPembimbing tr').each(function() {
            initMahasiswaAutocomplete($(this).find('.mahasiswa-autocomplete')[0]);
