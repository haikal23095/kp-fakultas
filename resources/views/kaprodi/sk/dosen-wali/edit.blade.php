...
@section('title', 'Edit SK Dosen Wali')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 fw-bold mb-0"><i class="fas fa-edit text-warning me-2"></i>Edit & Ajukan Ulang SK Dosen Wali</h1>
        <p class="mb-0 text-muted">Perbaiki data dan ajukan kembali SK yang ditolak</p>
    </div>
    <a href="{{ route('kaprodi.sk.dosen-wali.history') }}" class="btn btn-outline-secondary">
        <i class="fas fa-arrow-left me-2"></i>Kembali
    </a>
</div>

<form action="{{ route('kaprodi.sk.dosen-wali.update', $sk->No) }}" method="POST" id="formSKDosenWali">
    @csrf
    @method('PUT')
                    ...
                    <select class="form-select @error('id_prodi') is-invalid @enderror" 
                        ...
                        @foreach($prodis as $p)
                            <option value="{{ $p->Id_Prodi }}" 
                                    {{ $sk->Id_Prodi == $p->Id_Prodi ? 'selected' : '' }}>
                                {{ $p->Nama_Prodi }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle" id="tablePenetapan">
                    ...
                    <tbody id="bodyPenetapan">
                        @php
                            $rawData = $sk->Data_Dosen_Wali;
                            $dataDosenWali = is_string($rawData) ? json_decode($rawData, true) : (is_array($rawData) ? $rawData : []);
                        @endphp
                        @forelse($dataDosenWali as $index => $dosen)
                        <tr id="row-{{ $index + 1 }}">
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>
                                {{ $dosen['nama_dosen'] ?? 'N/A' }}
                                <input type="hidden" name="dosen[{{ $index + 1 }}][id_dosen]" value="{{ $dosen['id_dosen'] ?? '' }}">
                            </td>
                            <td>
                                <input type="number" 
                                       class="form-control form-control-sm" 
                                       name="dosen[{{ $index + 1 }}][jumlah_anak_wali]" 
                                       value="{{ $dosen['jumlah_anak_wali'] ?? 0 }}"
                                       min="0"
                                       required>
                            </td>
                            <td class="text-center">
                                <button type="button" class="btn btn-danger btn-sm btn-action" onclick="hapusDosen({{ $index + 1 }}, '{{ $dosen['id_dosen'] ?? '' }}')">
            ...
            <div class="d-flex align-items-start">
                ...
                <div>
                    ...
                    <ul class="mb-0 small text-muted">
                        ...
                        <li>SK akan diproses setelah diajukan ulang</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex justify-content-end gap-2 mb-4">
        <a href="{{ route('kaprodi.sk.dosen-wali.history') }}" class="btn btn-secondary">
            <i class="fas fa-times me-2"></i>Batal
        </a>
        <button type="submit" class="btn btn-warning" id="btnSubmit">
            <i class="fas fa-paper-plane me-2"></i>Ajukan Ulang SK Dosen Wali
        </button>
    </div>
</form>
...
<script>
    @php
        $existingDosenIds = collect($dataDosenWali)->pluck('id_dosen')->toArray();
    @endphp
    
    let dosenCounter = {{ count($dataDosenWali) }};
    let dosenList = @json($existingDosenIds);
