<aside class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" style="width: 280px; height: 100vh; position: fixed;">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
        ...
        {{-- MENU INI SUDAH DIPERBAIKI SESUAI ALUR KERJA --}}
        <li>
            <a href="{{ route('dosen.pengajuan.index') }}" class="nav-link text-white {{ request()->routeIs('dosen.pengajuan.*') ? 'active' : '' }}">
                <i class="fas fa-file-alt me-2"></i>
                Ajukan Surat Tugas
            </a>
        </li>
        <li>
            <a href="{{ route('dosen.riwayat.index') }}" class="nav-link text-white {{ request()->routeIs('dosen.riwayat.*') ? 'active' : '' }}">
    ...
    <div class="dropdown">
        <a href="#" class="d-flex align-items-center text-white text-decoration-none dropdown-toggle text-truncate w-100" id="dropdownUser1" data-bs-toggle="dropdown" aria-expanded="false" style="max-width: 240px;">
            ...
            <strong class="text-truncate">{{ auth()->user()?->Name_User ?? 'Dosen' }}</strong>
        </a>
