...
@section('title', 'SK Penguji Skripsi - Dekan')
...
<div class="page-header">
    <div class="d-flex align-items-center justify-content-between">
        <div>
            <h3 class="mb-1 fw-bold text-dark">SK Penguji Skripsi</h3>
            <p class="mb-0 text-muted small">Daftar SK Penguji Skripsi yang menunggu tanda tangan Dekan</p>
        </div>
    ...
    <div class="card-body p-0">
        @if($daftarSK->isEmpty())
            <div class="text-center py-5">
                ...
                <p class="text-muted">Tidak ada SK Penguji Skripsi yang menunggu persetujuan</p>
            </div>
        @else
        ...
        <div class="alert alert-info border-0" role="alert">
            <div class="d-flex align-items-start">
                ...
                <div>
                    ...
                    <ul class="mb-0 small">
                        <li>SK Penguji Skripsi yang muncul di sini adalah yang sudah disetujui oleh Wadek 1.</li>
        ...
        <div class="modal-content">
            <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title" id="modalDetailLabel">
                    <i class="fas fa-file-alt me-2"></i>Preview SK Penguji Skripsi
                </h5>
...
<script>
    ...
    function showHistory() {
        ...
        // Fetch history data
        fetch('{{ url('/dekan/sk-penguji-skripsi/history') }}', {
            method: 'GET',
            headers: {
    ...
    function renderHistory(historyList) {
        if (historyList.length === 0) {
            document.getElementById('historyContent').innerHTML = `
                <div class="text-center py-5">
                    <i class="fas fa-inbox fa-3x text-muted mb-3"></i>
                    <p class="text-muted">Belum ada SK Penguji Skripsi yang ditandatangani</p>
                </div>
            `;
            return;
        }
    ...
    function showHistoryDetail(skId) {
        ...
        // Fetch detail
        fetch(`{{ url('/dekan/sk-penguji-skripsi') }}/${skId}`, {
            method: 'GET',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
    ...
    function approveSK(skId, fromModal = false) {
        if (!confirm('Apakah Anda yakin ingin menyetujui dan menandatangani SK Penguji Skripsi ini?')) {
            return;
        }

        currentSKId = skId;
        
        fetch(`{{ url('/dekan/sk-penguji-skripsi') }}/${skId}/approve`, {
            method: 'POST',
            headers: {
    ...
    function submitRejection() {
        ...
        // Kirim request
        fetch(`{{ url('/dekan/sk-penguji-skripsi') }}/${currentRejectSKId}/reject`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
