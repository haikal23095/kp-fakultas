...
@section('title', 'SK Penguji Skripsi - Wadek 1')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 fw-bold mb-0">Daftar SK Penguji Skripsi</h1>
        <p class="mb-0 text-muted">SK Penguji Skripsi yang menunggu persetujuan dan history.</p>
    </div>
    <div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h6 class="m-0 fw-bold text-danger">SK Penguji Skripsi</h6>
    </div>
...
<script>
    ...
    function showDetail(skId) {
        ...
        .then(data => {
            ...
            console.log('Raw SK Data_Penguji_Skripsi:', data.sk.Data_Penguji_Skripsi);
            if (data.success) {
    ...
    function renderPreview(sk, dekanNameParam, dekanNipParam) {
        ...
        if (!Array.isArray(dataPenguji)) {
            console.error('Data_Penguji_Skripsi bukan array:', dataPenguji);
            dataPenguji = [];
        }
        
        console.log('Data Penguji (processed):', dataPenguji);
        
        // Debug: log first mahasiswa to see data structure
        if (dataPenguji.length > 0) {
            console.log('Sample mahasiswa data:', dataPenguji[0]);
            console.log('Penguji 1:', dataPenguji[0].penguji_1);
            console.log('Penguji 2:', dataPenguji[0].penguji_2);
            console.log('Penguji 3:', dataPenguji[0].penguji_3);
        }
                            ...
                            ${mahasiswaProdi.map((mhs, idx) => {
                                ...
                                <tr>
                                    ...
                                    <td style="font-size: 9pt;">
                                        ...
                                        <small>NIP: ${getNIP(mhs.penguji_1)}</small>
                                    </td>
                                    <td style="font-size: 9pt;">
                                        ${getNamaDosen(mhs.penguji_2)}<br>
                                        <small>NIP: ${getNIP(mhs.penguji_2)}</small>
                                    </td>
                                    <td style="font-size: 9pt;">
                                        ${getNamaDosen(mhs.penguji_3)}<br>
                                        <small>NIP: ${getNIP(mhs.penguji_3)}</small>
                                    </td>
                                </tr>
                                `;
                            }).join('')}
            ...
            <div style="text-align: justify; margin-bottom: 20px; font-size: 10pt;">
                ...
                <table style="width: 100%; margin-bottom: 15px;">
                    <tr>
                        ...
                        <td>PENETAPAN DOSEN PENGUJI SKRIPSI FAKULTAS TEKNIK UNIVERSITAS TRUNODJOYO SEMESTER ${semesterUpper} TAHUN AKADEMIK ${tahunAkademik}.</td>
                    </tr>
                </table>
    ...
    function approveSK(skId) {
        if (!confirm('Apakah Anda yakin ingin menyetujui SK Penguji Skripsi ini? SK akan diteruskan ke Dekan.')) {
            return;
        }

        fetch(`{{ url('/wadek1/sk-penguji-skripsi') }}/${skId}/approve`, {
            method: 'POST',
            headers: {
    ...
    function submitRejection() {
        ...
        fetch(`{{ url('/wadek1/sk-penguji-skripsi') }}/${skId}/reject`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
