...
@section('title', 'SK Beban Mengajar - Wadek 1')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="mb-0 text-muted">SK Beban Mengajar yang menunggu persetujuan dan history.</p>
    </div>
    <div>
                            ...
                            <td>
                                @php
                                    $badgeClass = match($sk->Status) {
                                        'Menunggu-Persetujuan-Wadek-1' => 'warning',
                                        'Menunggu-Persetujuan-Dekan' => 'info',
                                        'Selesai' => 'success',
                                        'Ditolak-Wadek1' => 'danger',
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="reject-to-admin">
                                <i class="fas fa-user-shield me-1"></i> Kembalikan ke Admin Fakultas untuk revisi
                            </label>
                        </div>
                        <div class="form-check mt-2">
                            ...
                            <label class="form-check-label" for="reject-to-kaprodi">
                                <i class="fas fa-user-tie me-1"></i> Tolak dan kembalikan ke Kaprodi
                            </label>
                        </div>
                    </div>
...
<script>
    ...
    function showDetail(skId) {
        ...
        fetch(`/wadek1/sk-beban-mengajar/${skId}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    displayDetail(data.sk, data.dekanName, data.dekanNip);
                } else {
    ...
    function displayDetail(sk, dekanName, dekanNip) {
        ...
        let html = `
            <div style="max-height: 750px; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 4px;">
                <div class="preview-document">
                    ...
                    <div style="text-align: center; margin: 15px 0; font-weight: bold; font-size: 11pt;">
                        BEBAN MENGAJAR DOSEN PROGRAM STUDI FAKULTAS TEKNIK<br>
                        UNIVERSITAS TRUNOJOYO MADURA<br>
                    ...
                    <div class="preview-content">
                        ...
                        <table style="width: 100%; margin-bottom: 10px; font-size: 10pt;">
                            <tr>
                                ...
                                <td style="width: 3%; vertical-align: top;">:</td>
                                <td style="text-align: justify;">Beban mengajar dosen Program Studi S1 Teknik Informatika, Program Studi S1 Teknik Industri, Program Studi S1 Teknik Elektro, Program Studi S1 Teknik Mesin, Program Studi S1 Sistem Informasi, Program Studi S1 Teknik Mekatronika Fakultas Teknik Universitas Trunojoyo Madura Semester ${semesterUpper} Tahun Akademik ${tahunAkademik} sebagaimana terlampir dalam surat keputusan ini.</td>
                            </tr>
                        </table>
                    ...
                    <div class="preview-signature" style="font-size: 10pt; margin: 40px 0 30px 0;">
                        ...
                        <p style="margin-bottom: 0;">
                            <strong><u>${dekanName || '[Nama Dekan]'}</u></strong><br>
                            NIP. ${dekanNip || '[NIP Dekan]'}
                        </p>
                    </div>
    ...
    function approveSK(skId) {
        if (!confirm('Apakah Anda yakin ingin menyetujui SK ini? SK akan diteruskan ke Dekan.')) {
            return;
        }

        fetch(`/wadek1/sk-beban-mengajar/${skId}/approve`, {
            method: 'POST',
            headers: {
    ...
    function showRejectModal(skId, semester, tahun) {
    ...
    function submitRejection() {
        ...
        fetch(`/wadek1/sk-beban-mengajar/${skId}/reject`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
